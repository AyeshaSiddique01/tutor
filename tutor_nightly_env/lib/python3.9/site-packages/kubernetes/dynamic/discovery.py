...
class Discoverer(object):
    ...
    def parse_api_groups(self, request_resources=False, update=False):
        ...
        if not self._cache.get("resources") or update:
            ...
            groups_response = self.client.request(
                "GET", "/{}".format(DISCOVERY_PREFIX)
            ).groups
    ...
    def _load_server_info(self):
        ...
        if not self._cache.get("version"):
            try:
                self._cache["version"] = {
                    "kubernetes": self.client.request(
...
class LazyDiscoverer(Discoverer):
    ...
    @property
    def api_groups(self):
        return self.parse_api_groups(request_resources=False, update=True)[
            "apis"
        ].keys()
    ...
    def __search(self, parts, resources, reqParams):
        ...
        if part != "*":
            ...
            if not resourcePart:
                ...
                if len(reqParams) != 2:
                    raise ValueError(
                        "prefix and group params should be present, have %s" % reqParams
                    )
