...
class PortForward:
    def __init__(self, websocket, ports):
        ...
        proxy = threading.Thread(
            name="Kubernetes port forward proxy: %s"
                    ...
                    while pending:
                        ...
                        if opcode == ABNF.OPCODE_BINARY:
                            ...
                            if channel >= len(channel_ports):
                                raise RuntimeError(
                                    "Unexpected channel number: %s" % channel
                                )
                            ...
                            if channel_initialized[channel]:
                                ...
                                port_number = six.byte2int(frame.data[1:2]) + (
                                    six.byte2int(frame.data[2:3]) * 256
                                )
                                if port_number != port.port_number:
                                    raise RuntimeError(
                                        "Unexpected port number in initial channel frame: %s"
                                        % port_number
                                    )
                            ...
                            raise RuntimeError(
                                "Unexpected websocket opcode: %s" % opcode
                            )
                        if not (
                            isinstance(self.websocket.sock, ssl.SSLSocket)
                            and self.websocket.sock.pending()
                        ):
                            pending = False
                else:
...
def websocket_proxycare(connect_opt, configuration, url, headers):
    ...
    if configuration.proxy:
        ...
        connect_opt.update(
            {"http_proxy_host": proxy_url.hostname, "http_proxy_port": proxy_url.port}
        )
        ...
        if param == "ports":
            for port in value.split(","):
                ...
                if not (0 < port_number < 65536):
                    raise ApiValueError(
                        "Port number must be between 0 and 65536: %s" % port
                    )
